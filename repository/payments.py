...
from routers.payments.pgw2c2p.services import (
    get_teleconsult_payment_token,
    authorize_payment_2c2p,
    should_use_authorization_flow
)
from services.teleconsult import PaymentTotal
from utils.stripe import fetch_payment_sheet, generate_stripe_paynow_link
from config import FALLBACK_TO_IMMEDIATE_CHARGE_ON_AUTH_FAILURE
import logging
...
def create_prepayment(
    ...
    # Stripe PayNow Flow
        ...
        patient_type_str = teleconsults[0].patient_type.value if teleconsults and hasattr(teleconsults[0], 'patient_type') else PatientType.PRIVATE_PATIENT.value
        
        # Check if we should use authorization/capture flow
        use_authorization = should_use_authorization_flow(user, patient_type_str)
        
        if use_authorization:
            try:
                # Try authorization flow (hold funds without charging)
                logging.info(f"Using authorization flow for user {user.id}, amount ${txn_amount:.2f}")
                payment_token, payment_id, payment_record = authorize_payment_2c2p(
                    db, user, txn_amount, card_token, PaymentType.PREPAYMENT,
                    description=f"Teleconsultation Prepayment"
                )
                # Payment record already created by authorize_payment_2c2p
                # We'll associate it with teleconsults below
                authorization_mode = True
                
            except Exception as auth_error:
                logging.error(f"Authorization failed for user {user.id}: {auth_error}")
                
                # Fallback to immediate charge if enabled
                if FALLBACK_TO_IMMEDIATE_CHARGE_ON_AUTH_FAILURE:
                    logging.warning(f"Falling back to immediate charge for user {user.id}")
                    payment_token, payment_id = get_teleconsult_payment_token(
                        db, user, txn_amount, card_token, PaymentType.PREPAYMENT
                    )
                    authorization_mode = False
                else:
                    # Re-raise error if fallback disabled
                    raise
        else:
            # Use immediate charge flow (old behavior)
            logging.info(f"Using immediate charge flow for user {user.id}, amount ${txn_amount:.2f}")
            payment_token, payment_id = get_teleconsult_payment_token(
                db, user, txn_amount, card_token, PaymentType.PREPAYMENT
            )
            authorization_mode = False
        ...
        payment_provider_params = {
            ...
            "authorization_mode": authorization_mode  # Tell mobile app if this is auth or charge
        }
