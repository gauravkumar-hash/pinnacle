...
class AppointmentCorporateQuotaUsage(Base):
    """Track daily and total appointment quota usage per corporate code"""
    __tablename__ = "appointment_corporate_quota_usage"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    corporate_code_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("appointment_corporate_codes.id"), index=True)
    date: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True)  # Date for daily tracking
    appointments_count: Mapped[int] = mapped_column(Integer, default=0)  # Number of appointments for this date
...
class AppointmentCorporateCode(Base):
    ...
    category: Mapped[AppointmentCategory] = mapped_column(default=AppointmentCategory.GENERAL)
    # Appointment quota control (appointments only, not telemedicine)
    max_appointments_total: Mapped[Optional[int]] = mapped_column(Integer)  # Total appointment limit
    max_appointments_per_day: Mapped[Optional[int]] = mapped_column(Integer)  # Daily appointment limit
...
class AppointmentBranchOperatingHours(Base):
    ...
    max_appointments_per_session: Mapped[Optional[int]] = mapped_column(Integer)  # Control appointments per session
    # Branch relationship
