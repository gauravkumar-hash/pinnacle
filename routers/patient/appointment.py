...
def get_appointment_timings(req: GetAppointmentTimingsReq, db: Session = Depends(get_db)):
    ...
    available_slots, appt_operating_hours_discrete_max_bookings, max_appts_per_session = get_appointment_operating_hours(db, branch, start_date, end_date)
    ...
    booked_slots = get_appointment_booked_slots(db, branch, start_date, end_date, available_slots, appt_operating_hours_discrete_max_bookings, max_appts_per_session)
...
def get_price(req: GetPriceBreakdownReq, db: Session = Depends(get_db), user: Account = Depends(validate_user)):
    ...
    # Check Corporate Code Quotas
    if corporate_code_record:
        num_patients = len(req.patients)
        # Count total confirmed/completed appointments using this corporate code
        if corporate_code_record.max_appointments_total is not None:
            total_used = db.query(func.count(Appointment.id)).filter(
            ...
            if total_used + num_patients > corporate_code_record.max_appointments_total:
