...
class CorporateCodeBase(BaseModel):
    ...
    is_active: bool = True
    category: AppointmentCategory = AppointmentCategory.GENERAL
    max_appointments_total: int | None = None  # Total appointment quota
    max_appointments_per_day: int | None = None  # Daily appointment quota
...
def get_corporate_codes(
    ...
    for corp_code in corporate_codes:
        ...
        result.append(CorporateCodeDetails(
            ...
            max_appointments_total=corp_code.max_appointments_total,
            max_appointments_per_day=corp_code.max_appointments_per_day,
...
def create_corporate_code(req: CorporateCodeCreate, db: Session = Depends(get_db)):
    ...
    corporate_code = AppointmentCorporateCode(
        ...
        max_appointments_total=req.max_appointments_total,
        max_appointments_per_day=req.max_appointments_per_day
    )
...
def update_corporate_code(code_id: str, req: CorporateCodeUpdate, db: Session = Depends(get_db)):
    ...
    if "max_appointments_total" in req.model_fields_set:
        corp_code.max_appointments_total = req.max_appointments_total
    if "max_appointments_per_day" in req.model_fields_set:
        corp_code.max_appointments_per_day = req.max_appointments_per_day
...
# Corporate Code Quota Usage Endpoint
class CorporateCodeQuotaUsage(BaseModel):
    corporate_code_id: str
    code: str
    organization: str
    max_appointments_total: int | None
    max_appointments_per_day: int | None
    total_appointments_used: int
    today_appointments_used: int
    quota_remaining_total: int | None
    quota_remaining_today: int | None

@router.get("/corporate-codes/{code_id}/quota-usage", response_model=CorporateCodeQuotaUsage)
def get_corporate_code_quota_usage(code_id: str, db: Session = Depends(get_db)):
    ...
    # Count total appointments with this corporate code
    ...
    quota_remaining_total = None
    if corp_code.max_appointments_total:
        quota_remaining_total = max(0, corp_code.max_appointments_total - total_appointments)

    quota_remaining_today = None
    if corp_code.max_appointments_per_day:
        quota_remaining_today = max(0, corp_code.max_appointments_per_day - today_appointments)

    return CorporateCodeQuotaUsage(
        corporate_code_id=str(corp_code.id),
        code=corp_code.code,
        organization=corp_code.organization,
        max_appointments_total=corp_code.max_appointments_total,
        max_appointments_per_day=corp_code.max_appointments_per_day,
        total_appointments_used=total_appointments,
        today_appointments_used=today_appointments,
        quota_remaining_total=quota_remaining_total,
...
class OperatingHourBase(BaseModel):
    ...
    cutoff_time: int = 0
    max_bookings: int = 1
    max_appointments_per_session: int | None = None  # Control appointments per session
    branch_id: str
        ...
        for hour_data in hours_list:
            ...
            if model_class == AppointmentBranchOperatingHours:
                operating_hour = AppointmentBranchOperatingHours(
                    ...
                    max_appointments_per_session=hour_data.get('max_appointments_per_session'),
...
class SessionSlotCapacity(BaseModel):
    slot_id: str
    day: str
    start_time: str
    end_time: str
    max_appointments_per_session: int | None
    booked_count: int
    available: int | None  # None means unlimited
    is_full: bool


class BranchSessionCapacityResponse(BaseModel):
    branch_id: str
    branch_name: str
    date: str
    slots: list[SessionSlotCapacity]


@router.get("/operating-hours/{branch_id}/session-capacity", response_model=BranchSessionCapacityResponse)
def get_branch_session_capacity(
    branch_id: str,
    ...
    branch = db.query(Branch).filter(Branch.id == branch_id).first()
    if not branch:
        raise HTTPJSONException(title="Not Found", message="Branch not found", status_code=404)

    # Parse date or default to today in SG time
    if date:
        try:
            check_date = datetime.strptime(date, "%Y-%m-%d").date()
        except ValueError:
            raise HTTPJSONException(title="Invalid Date", message="Date must be in YYYY-MM-DD format", status_code=400)
    else:
        check_date = datetime.now(sgtz).date()

    # Map Python weekday to DayOfWeek enum (Monday=0)
    weekday_map = {0: "Monday", 1: "Tuesday", 2: "Wednesday", 3: "Thursday",
                   4: "Friday", 5: "Saturday", 6: "Sunday"}
    day_name = weekday_map[check_date.weekday()]

    # Fetch operating hour slots for this branch and day
    operating_hours = db.query(AppointmentBranchOperatingHours).filter(
        AppointmentBranchOperatingHours.branch_id == branch_id,
        AppointmentBranchOperatingHours.day == DayOfWeek(day_name)
    ).all()

    slots = []
    for oh in operating_hours:
        # Count appointments on this date that start within this slot window
        # branch is stored as JSON with "id" key
        slot_start = datetime.combine(check_date, oh.start_time).replace(tzinfo=sgtz)
        slot_end = datetime.combine(check_date, oh.end_time).replace(tzinfo=sgtz)

        booked_count = db.query(func.count(Appointment.id)).filter(
            Appointment.start_datetime >= slot_start,
            Appointment.start_datetime < slot_end,
            Appointment.status.not_in([AppointmentStatus.CANCELLED]),
            func.cast(Appointment.branch["id"].astext, String) == str(branch_id)
        ).scalar() or 0

        max_cap = oh.max_appointments_per_session
        available = (max_cap - booked_count) if max_cap is not None else None
        is_full = (available is not None and available <= 0)

        slots.append(SessionSlotCapacity(
            slot_id=str(oh.id),
            day=day_name,
            start_time=oh.start_time.strftime("%H:%M"),
            end_time=oh.end_time.strftime("%H:%M"),
            max_appointments_per_session=max_cap,
            booked_count=booked_count,
            available=available,
            is_full=is_full,
        ))

    return BranchSessionCapacityResponse(
        branch_id=str(branch.id),
        branch_name=branch.name,
        date=str(check_date),
        slots=slots,
...
def create_operating_hour(req: OperatingHourCreate, db: Session = Depends(get_db)):
    ...
    operating_hour = AppointmentBranchOperatingHours(
        ...
        max_appointments_per_session=req.max_appointments_per_session,
